...
class Construct:
    ...
    def __init__(self, layers=None, str_activation_functions=None, optimizers=None, shape=None,
                 ...
                 learning_rate_adam=0.001, rho1=0.9, rho2=0.999, Loss_function="MSE"):
    ...
    def Resilient_prop_update(self, gradient, index):
        ...
        self.ILG[index][1] = self.ILG[index][1] * ((np.sign(gradient * self.ILG[index][2]) *
                                                    (self.learning_rate_up - self.learning_rate_down) + np.abs(
                    np.sign(gradient *
                            self.ILG[index][2])) * (self.learning_rate_up + self.learning_rate_down)) / 2 +
                                                   np.abs(self.ILG[index][0] - np.abs(
                                                       np.sign(gradient * self.ILG[index][2]))))
    ...
    def forward(self, input_vector, old_eval=False):
        # Bias neuron
    ...
    def sum(self):
        return sum([sum([sum([np.abs(weight) for weight in neuron])
